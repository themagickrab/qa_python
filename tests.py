...
class TestBooksCollector:
    ...
    def test_add_book_twice(self, add_book): #тест нельзя добавить одну и ту же книгу дважды
        add_book.add_new_book('Гордость и предубеждение и зомби')
        collector = add_book.get_books_rating()
        assert len(collector) == 1

    def test_impossible_to_set_rating_for_book_that_is_not_in_the_list(self, add_book): #Нельзя выставить рейтинг книге, которой нет в списке
    ...
    def test_you_cannot_set_rating_less_than_one(self, add_book): #Нельзя выставить рейтинг меньше 1
    ...
    def test_you_cannot_set_score_greater_than_10(self, add_book): #Нельзя выставить рейтинг больше 10
        add_book.set_book_rating('Гордость и предубеждение и зомби', 11)
        rating_list = add_book.get_books_with_specific_rating(11)
        assert len(rating_list) == 0

    def test_book_that_was_not_added_has_no_rating(self, add_book): #У не добавленной книги нет рейтинга.
    ...
    def test_adding_book_favorites(self, add_book): #Добавление книги в избранное
    ...
    def test_cannot_add_book_to_favorites_if_not_in_books_rating_dictionary(self, add_book): #Нельзя добавить книгу в избранное, если её нет в словаре books_rating
    ...
    def test_deleting_book_from_favorites(self,add_book): #Проверка удаления книги из избранного
        add_book.add_book_in_favorites('Гордость и предубеждение и зомби')
